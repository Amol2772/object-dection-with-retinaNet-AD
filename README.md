# Object-Detection-With-RetinaNet-AS
...
```
# 8. Building the ResNet50 backbone
```def get_backbone():
...
```
# 9.Building Feature Pyramid Network as a custom layer
...
```
# 10.Building the classification and box regression heads.
```def build_head(output_filters, bias_init):
    ...
    return head
```
# 11.Building RetinaNet using a subclassed model
...
```
# 12.Implementing a custom layer to decode predictions 
...
```
# 13. Implementing Smooth L1 loss and Focal Loss as keras custom losses
...
class RetinaNetLoss(tf.losses.Loss):
    ...
    def call(self, y_true, y_pred):
        ...
        return loss
```
# 14.Setting up training parameters
```model_dir = "retinanet/"
...
```
# 15.Initializing and compiling model
...
```
# 17.Load the COCO2017 dataset using TensorFlow Datasets
```
(train_dataset, val_dataset), dataset_info = tfds.load(
    "coco/2017", split=["train", "validation"], with_info=True, data_dir="data"
)
```
# 18.Setting up a tf.data pipeline
